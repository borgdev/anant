...
def node_degree_centrality(
    ...
    # Count edges per node (fix column names)
    node_degrees = (
        incidence_df
        .group_by("node_id")
...
def closeness_centrality(
    ...
    node_edges = (
        incidence_df
        .group_by("node_id")
        .agg(pl.col("edge_id").alias("edge_list"))
    )
    
    node_to_edges = dict(zip(
        node_edges["node_id"].to_list(),
...
def s_centrality(
    ...
    centralities_dict = dict(zip(
        s_centralities["node_id"].to_list(),
    ...
    if normalized and hg.num_edges > 0:
        ...
        max_edge_size = max(hg.get_edge_size(edge) for edge in hg.edges)
...
def weighted_degree_centrality(
    ...
    edge_weight_list = [
        {"edge_id": edge, "weight": edge_weights.get(edge, 1.0)}
        for edge in hg.edges
    ]
    ...
    weights_df = weights_df.with_columns([
        pl.col("edge_id").cast(incidence_df["edge_id"].dtype)
    ])
    
    # Join with incidence and sum weights per node
    weighted_degrees = (
        incidence_df
        .join(weights_df, on="edge_id")
        .group_by("node_id")
    ...
    # Convert to dictionary
    centralities_dict = dict(zip(
        weighted_degrees["node_id"].to_list(),
